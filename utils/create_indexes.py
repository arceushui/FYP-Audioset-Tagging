...
def combine_full_indexes_project(args):
    """Combine all balanced and unbalanced indexes hdf5s to a single hdf5. This
    combined indexes hdf5 is used for training with full data (~20k balanced
    audio clips + ~1.9m unbalanced audio clips).
    """
    ...
    indexes_hdf5s_dir = args.indexes_hdf5s_dir
    full_indexes_hdf5_path = args.full_indexes_hdf5_path
    split_id_list_to_train = args.split_id_list_to_train.split(',')
    #classes_num = config.classes_num
    classes_num = 9
    ...
    paths = get_sub_filepaths(indexes_hdf5s_dir)
    #paths = [path for path in paths if (
    #    'split' in path and 'full_train' not in path and 'mini' not in path)]
    paths_2 = []
    for path in paths:
        if 'project' in path:
            for id in split_id_list_to_train:
                if id in path.split('project')[1]:
                    paths_2.append(path)
    paths = paths_2
    ...
    with h5py.File(full_indexes_hdf5_path, 'w') as full_hf:
        ...
        for path in paths:
            with h5py.File(path, 'r') as part_hf:
                ...
                full_hf['target'].resize((new_n, classes_num))
                full_hf['target'][n : new_n] = part_hf['target'][:]
                for ls in (full_hf['target'][n : new_n]).tolist():
                    print(ls)
...
if __name__ == '__main__':
    ...
    parser_combine_full_indexes_project = subparsers.add_parser('combine_full_indexes_project')
    parser_combine_full_indexes_project.add_argument('--indexes_hdf5s_dir', type=str, required=True, help='Directory containing indexes hdf5s to be combined.')
    parser_combine_full_indexes_project.add_argument('--full_indexes_hdf5_path', type=str, required=True, help='Path to write out full indexes hdf5 file.')
    parser_combine_full_indexes_project.add_argument('--split_id_list_to_train', type=str, required=True)
    ...
    elif args.mode == 'combine_full_indexes_project':
        combine_full_indexes_project(args)
